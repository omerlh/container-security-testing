...
function run_tests()
{
  dotnet test -c Release --no-build ./tests.csproj
}
...
then
  ...
  curl -s --fail $PROXY_URL/OTHER/core/other/jsonreport/?formMethod=GET --output /reports/report.json
  curl -s --fail $PROXY_URL/OTHER/core/other/htmlreport/?formMethod=GET --output /reports/report.html
